####
# Code for getting data for Touhue
# JMH 14 Mar 21
...
ZPmonth <- ZP2 %>% 
  mutate(M = as.factor(as.character(strftime(Sample_date, format = "%m"))),
         Y = as.factor(as.character(strftime(Sample_date, format = "%Y")))) %>% 
  group_by(Y, M, Sample_site, Life_stage2, Genus_sp_95toPresent, Order1) %>% 
  summarize(Number_counted = sum(Number_counted, na.rm=T),
           Zoop_density = sum(Zoop_density, na.rm=T),
           Zoop_biomass = sum(Zoop_biomass, na.rm=T))
...
#Check data
summary(Fin, maxsum = 100)

# are there dups
FinCheck <- Fin %>% 
  mutate(key = paste0(Y, "_", M, "_", Sample_site, "_",Life_stage2, "_",Genus_sp_95toPresent),
         dups = duplicated(key))

# none
FinCheck[FinCheck$dups == TRUE, ]


write.csv(Fin, file.path(here::here("03_exports","YangDataRequest20210314.csv")))
# save.image(file.path(here::here("04_SavedRimages","YangDataRequest20210314_Rdat")))
