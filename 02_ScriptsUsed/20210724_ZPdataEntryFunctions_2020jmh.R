...
step0.5 <- function(Files){
  ...
  species_num <- sum(!is.na(data$Number_counted)) #DOD
  # species_num <- length(data[!is.na(data$Number_counted),]$Genus_sp_lifestage) - JMH
  ...
  #OLD VERSION: Zlg_db <- read.csv("/06_RawZPdata/DO_NOT_DELETE_Copepod_length_cutoffs_for_entry_20200117.csv") %>% 
  #   mutate_if(is.factor, as.character)
  Zlg_db <- read.csv(file.path(here::here("06_RawZPdata"), "DO_NOT_DELETE_CopLgCutoffs_20200117.csv")) %>% 
    mutate_if(is.factor, as.character)
  ...
  Prop_ls <- lengths0 %>% 
    ...
    mutate(Prop_O = replace(Prop_O, is.nan(Prop_O), NA),
           ...
           Prop_O = replace(Prop_O, grepl("unclass", Genus_sp_lifestage) & !is.na(Prop_O), 0),
           Prop_Y = replace(Prop_Y, grepl("unclass", Genus_sp_lifestage) & !is.na(Prop_Y), 1))
  ...
  lengths2 <- lengths %>% 
    ...
    mutate(Life_stage = replace(Life_stage, !is.na(Length_cutoff) & Length_mm < Length_cutoff, "Younger"),
           ...
           Life_stage = replace(Life_stage, grepl("unclass",Genus_sp_lifestage), "Younger")) %>% 
  ...
  counts2 <- counts %>% 
    ...
    mutate(GC4_count = ifelse(!grepl("immature",Genus_sp_lifestage) & !grepl("unclass",Genus_sp_lifestage), round(Number_counted*Prop_O), 0),
           LC4_count = ifelse(!grepl("immature",Genus_sp_lifestage) & !grepl("unclass",Genus_sp_lifestage), round(Number_counted*Prop_Y), Number_counted),
  ...
  Young <- counts2 %>% 
    filter(Genus_sp_lifestage %in% Zlg_db$Genus_sp_lifestage|grepl("immature",Genus_sp_lifestage)|grepl("unclass",Genus_sp_lifestage)) %>% 
  ...
  table_list <<- append(table_list, list(dat=list("SI"=SI,"Zoop_counts"=Zoop_counts,"Zoop_lengths"=Zoop_lengths,
                                                  "Zoop_eggs"=Zoop_eggs, "File"=File, "Datasheet"=data)))
...
#DOD version - seems to throw an error when there are younger and older created, which seems like a problem - REMOVED
  # SpecCt_OK <- ifelse(species_num != sum(Zoop_counts$Number_counted != 0), paste0("Species counts NOT OK, Check datasheet ",counter), 
  #                     "Species counts OK")
  
  # JMH version - UPDATED 25 JUL 2021
  Sp_numRaw <- length(unique(data[!is.na(data$Number_counted),]$Genus_sp_lifestage))
  Sp_numFinal <- length(unique(Zoop_counts[Zoop_counts$Number_counted != 0,]$Genus_sp_lifestage))
  SpecCt_OK <- ifelse(Sp_numRaw != Sp_numFinal, paste0("Species counts NOT OK, Check datasheet ",counter),
                      "Species counts OK")
...
## Needed in teh following function.
ZF_list <- list()
...
step1.5 <- function(table_list){
  ...
  ZF_list <<- append(ZF_list, list(dat=list("Zoop_final"=Zoop_fin, "Datasheet"=table_list$Datasheet, "File"=table_list$File)))
...
input1.5 <- function(ZF_list){
  
  ## How many raw datasheets have been entered?
  counter4 <<- counter4+1
  
  Zoop_final <- ZF_list$Zoop_final
  ...
  dbBegin(zoop)
    ...
    ## Add the relevant data to the permanent Zoop_final table in LEPAS_2020.db.
    dbExecute(zoop, "INSERT INTO 
  ...
  datasheet <- ZF_list$Datasheet
  
  ## Write a permanent copy of the raw datasheet to the "Entered" folder. 
  write.csv(datasheet, paste0(file.path(here::here("06_RawZPdata/2020/WB")), "/Entered/", ZF_list$File[1,1]), na="")
  ...
  print(paste0("Zoop_final ", counter4, " entered into LEPAS_2020.db."))
...
input2 <- function(x){
  counter4 <<- 0
  ifelse(x=="OK", lapply(ZF_list, input1.5), "Please check data and enter 'OK' into input2.")
} %>% invisible()
