...
input0.5 <- function(table_list){
  ...
  dbBegin(zoop)
    ...
    ## Add the relevant sample info to the permanent Zoop_sample_info table in LEPAS_2020 - Copy.db.
...
input1.5 <- function(table_list){
  ...
  datasheet <- table_list$Datasheet
  
  ## Write a permanent copy of the raw datasheet to the "Entered" folder. 
  write.csv(datasheet, paste0("Z:\\Projects\\FADX09_LEPAS\\15-LEPAS Datasheets\\Entered Spreadsheets\\3-Zooplankton\\LEPAS_", Year,"\\",Year,"-",Basin,"\\Entered\\",
                         table_list$File[1,1]), na="")
